...
struct cursor cursor_ID;
struct cursor cursor_PW;
int fd;
int size;
char *map;
int exit_State;
...
void draw_alert_box(const char *message, int x, int y, int w, int h,
  ...
  draw_text(message, x + w / 5, y + h / 3, 4, text_color, vip, fip, map);
...
// 사용자 인증
int authenticate_user(const char *userID, const char *password,
                      struct fb_var_screeninfo *vip,
                      struct fb_fix_screeninfo *fip, char *map) {
  ...
  snprintf(query, sizeof(query),
           "SELECT COUNT(*) FROM tbl_name WHERE ID='%s' AND PW='%s';", userID,
           password);
  ...
  int auth_success = atoi(row[0]) > 0;
  mysql_free_result(result);
  db_disconnect(conn);

  if (!auth_success) {
    // 로그인 실패 → "Login failed" 경고 박스 표시 후 2초 후 자동 삭제 (화면
    // 복원)
    draw_alert_box("LOGIN FAILED", 250, 200, 300, 100, COLOR_RED, COLOR_WHITE,
                   vip, fip, map);
  }
...
void handle_touch_event(int screen_x, int screen_y, bool touch_down,
  ...
  if (*IP_State == ID) {
    draw_rounded_rect(160, 200, 300, 60, 15, COLOR_MAIN, &vinfo, &finfo, map);
    draw_text(touch_Val, 165, 212, 5, COLOR_WHITE, &vinfo, &finfo, map);
  } else if (*IP_State == PW) {
    draw_rounded_rect(160, 300, 300, 60, 15, COLOR_MAIN, &vinfo, &finfo, map);

    for (int i = 0; i < *touch_State; i++) {
      draw_rounded_rect(165 + (i * 30), 310, 25, 40, 5, COLOR_WHITE, &vinfo,
                        &finfo, map);
    }
  }

  if (screen_x >= 165 && screen_x <= 400 && screen_y >= 200 &&
      screen_y <= 270) {
    printf("Button ID pressed\n");
    *IP_State = ID;
  } else if (screen_x >= 165 && screen_x <= 400 && screen_y >= 320 &&
             screen_y <= 390) {
    printf("Button PW pressed\n");
    *IP_State = PW;
  }
...
void *thread_func1(void *arg) {
  for (;;) {
    if (IP_State == ID) {
      draw_letter('I', cursor_ID.x, cursor_ID.y, 5, COLOR_WHITE, &vinfo, &finfo,
                  map);
      sleep(1);
      draw_letter('I', cursor_ID.x, cursor_ID.y, 5, COLOR_MAIN, &vinfo, &finfo,
                  map);
      sleep(1);
    } else if (IP_State == PW) {
      draw_letter('I', cursor_PW.x, cursor_PW.y, 5, COLOR_WHITE, &vinfo, &finfo,
                  map);
      sleep(1);
      draw_letter('I', cursor_PW.x, cursor_PW.y, 5, COLOR_MAIN, &vinfo, &finfo,
                  map);
      sleep(1);
    }
      ...
      if (ev.code == BTN_TOUCH && ev.value == 0) {  // 터치가 떼어졌을 때
        ...
        if (IP_State == ID) {
          handle_touch_event(screen_x, screen_y, ev.code == BTN_TOUCH,
                             &touch_StateID, touch_ValID, &IP_State,
                             &cursor_ID);
        } else if (IP_State == PW) {
          handle_touch_event(screen_x, screen_y, ev.code == BTN_TOUCH,
                             &touch_StatePW, touch_ValPW, &IP_State,
                             &cursor_PW);
        }

        if (exit_State == CHECK) {
          // 여기서 db의 아이디와 비밀번호가 일치하는지 체크 한다.

          if (authenticate_user(touch_ValID, touch_ValPW, &vinfo, &finfo,
                                map)) {
            exit_State = OK;
...
int main(int argc, char *argv[]) {
  ...
  if (pid == 0) {
    // 자식 프로세스: 카메라 실행
    execl("./intel_bank_atm_camera", "./intel_bank_atm_camera", NULL);
    perror("execl failed");
    return 1;
  } else if (pid > 0) {
    // 부모 프로세스: ATM UI 실행
    printf("Camera started with PID: %d\n", pid);
    // main logic 실행
  } else {
    perror("fork failed");
    return 1;
  ...
  draw_text("WELCOME TO OHSY BANK", 70, 65, 4, COLOR_WHITE, &vinfo, &finfo,
            map);

  draw_rounded_rect(160, 200, 300, 60, 15, COLOR_MAIN, &vinfo, &finfo, map);
  draw_text("ID", 30, 215, 4, COLOR_BLACK, &vinfo, &finfo, map);

  draw_rounded_rect(160, 300, 300, 60, 15, COLOR_MAIN, &vinfo, &finfo, map);
  draw_text("PASSWORD", 30, 315, 4, COLOR_BLACK, &vinfo, &finfo, map);
  ...
  printf("[%d] creating thread\n", pid);
  ret = pthread_create(&thread_id2, NULL, thread_func2, NULL);
  if (ret != 0) {
    printf("[%d] error: %d (%d)\n", pid, ret, __LINE__);
    return EXIT_FAILURE;
  }
  ...
  execl("./intel_bank_atm_main", "./intel_bank_atm_main", touch_ValID, NULL);
